...
void Solution::search_layer_build(const float* query, vector<int>& candidates, 
    ...
    while (!C.empty()) {
        ...
        if (dist_c > W.top().first) break; // 剪枝
        ...
        for (size_t i = 0; i < neighbors.size(); ++i) {
            int nid = neighbors[i];
            if (tls_visited.is_visited(nid)) continue;
...
void Solution::build(int d, const vector<float>& base) {
    ...
    for(int i=0; i<num_vectors; ++i) omp_init_lock(&node_locks[i]);
        ...
        for (int i = 1; i < num_vectors; ++i) {
            ...
            for (int lc = min(level, cur_max_level); lc >= 0; --lc) {
                ...
                for (const auto& pair : sorted_cand) {
                    if (selected_neighbors.size() >= (size_t)M_limit) break;
                ...
                for (int neighbor_id : selected_neighbors) {
                    ...
                    if (need_prune) {
                        ...
                        for (const auto& pair : t_cand) {
                            if (new_conn.size() >= (size_t)M_limit) break;
                            ...
                            if (good) new_conn.push_back(c_id);
                        }
    ...
    // 清理锁
    for(int i=0; i<num_vectors; ++i) omp_destroy_lock(&node_locks[i]);
